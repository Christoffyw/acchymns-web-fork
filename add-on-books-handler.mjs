...
const pre_packaged_books = ["ZH", "GH", "HG", "JH"];
const mappings = {
...
for (const book of books) {
    ...
    if (process.env.CAPACITOR_PLATFORM_NAME != "web" && !pre_packaged_books.includes(book)) {
        // Don't include book in final bundle if deploying to android or ios
        fs.rmSync(path.join(target_dir, book, "songs"), { recursive: true });
        console.log(book, "songs removed from bundle");
    }
}
